...
class App(QMainWindow):
    ...
    def update_distribution_graph(self):
        ...
        if self.class_map != None:
            ...
            self.ax.clear()
            self.ax.barh(self.class_labels, self.percentages, color=self.class_colours)
    ...
    def calculate_distribution(self):
        ...
        size = (np.array(self.class_map[self.slider.value()]).shape[0] - 1) * (
            np.array(self.class_map[self.slider.value()]).shape[1] - 1
        )
        for i in range(len(self.class_map[self.slider.value()]) - 1):
            for j in range(len(self.class_map[self.slider.value()][i]) - 1):
                self.percentages[int(self.class_map[self.slider.value()][i][j])] += (
                    1 / size
                )
    ...
    def create_bar_chart(self, layout):
        ...
        self.ax.barh(
            self.class_labels,
            np.zeros(len(self.class_labels)),
    ...
    def load_btn_clicked(self):
        ...
        self.model = keras.models.load_model(
            f"{MODEL_PATH}/{self.model_selection_dropdown.currentText()}"
        )
        ...
        if self.class_map != None:
            ...
            for i in range(len(self.class_map[self.slider.value()]) - 1):
                for j in range(len(self.class_map[self.slider.value()][i]) - 1):
                    colour = QColor(
                        self.class_colours[
                            int(self.class_map[self.slider.value()][i][j])
                        ]
                    )
                    colour.setAlpha(ALPHA)
                    painter.fillRect(
                        j * BLOCK_SIZE, i * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE, colour
                    )
            painter.end()
    ...
    def download_button_clicked(self):
        ...
        north, south, east, west = postcode_to_area(
            self.location, self.height, self.width
        )
    ...
    def download_error(self, err):
        ...
        self.error_dialog.showMessage(
            "Too many requests were made. Wait before reattempting download."
        )
    ...
    def classify(self):
        ...
        if "_ms_" in self.model_selection_dropdown.currentText():
            ...
            try:
                ...
                self.error_dialog.showMessage(
                    "Data download does not contain all 12 spectral bands. Therefore cannot be used with multispectral classification. Use RGB classifier instead"
                )
        else:
            print("RGB Classify")
            worker = Worker(
